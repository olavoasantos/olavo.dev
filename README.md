...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create olavo-dev
  fly apps create olavo-dev-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app olavo-dev
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app olavo-dev-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'olavo-dev-staging' does not match app name in config file 'olavo-dev'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name olavo-dev-db
  fly postgres attach --app olavo-dev olavo-dev-db

  fly postgres create --name olavo-dev-staging-db
  fly postgres attach --app olavo-dev-staging olavo-dev-staging-db
  ```
